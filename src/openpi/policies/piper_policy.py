...
class PiperInputs(transforms.DataTransformFn):
    ...
    target_dim: int = 32
    image_keys: Tuple[str, ...] = ("base_0_rgb", "wrist_0_rgb")
    ...
    def __call__(self, data: dict) -> dict:
        ...
        base_present = ("base_0_rgb" in img) and (img["base_0_rgb"] is not None)
        if not base_present:
            raise KeyError("Missing required base camera: image['base_0_rgb'].")
        base_image = _to_numpy_image(img["base_0_rgb"])

        wrist_present = ("wrist_0_rgb" in img) and (img["wrist_0_rgb"] is not None)
        if not wrist_present:
            raise KeyError("Missing required wrist camera: image['wrist_0_rgb'].")
        wrist_image = _to_numpy_image(img["wrist_0_rgb"])

        # pi0/pi05 expects three fixed camera keys.
        # For 2-view input (base + wrist), map wrist -> left_wrist and pad right_wrist with zeros.
        # Keep right_wrist masked off for non-FAST models.
